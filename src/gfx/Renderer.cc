...
void Renderer::allocate_command_buffers(Context const& ctx, std::size_t count) {
  command_buffers_.resize(count, VK_NULL_HANDLE);
...
void Renderer::create_sync(Context const& ctx) {
  image_available_.resize(kMaxFramesInFlight, VK_NULL_HANDLE);
  render_finished_.resize(kMaxFramesInFlight, VK_NULL_HANDLE);
  in_flight_.resize(kMaxFramesInFlight, VK_NULL_HANDLE);
...
void Renderer::create_framebuffers(Context const& ctx, Swapchain const& sc, Pipeline const& pl) {
  framebuffers_.resize(sc.image_views().size(), VK_NULL_HANDLE);
...
void Renderer::record_command_buffer(VkCommandBuffer cb, Swapchain const& sc, Pipeline const& pl, VkFramebuffer fb) {
  ...
  vkCmdPushConstants(cb, pl.pipeline_layout(), VK_SHADER_STAGE_VERTEX_BIT, 0, static_cast<uint32_t>(sizeof(float)), &aspect);
...
bool Renderer::draw_frame(Context const& ctx, GLFWwindow* window, Swapchain& sc, Pipeline& pl) {
  if (window == nullptr) {
    fail("Renderer::draw_frame: window == nullptr");
  }

  VkFence const fence = in_flight_[frame_index_];
  ...
  VkCommandBuffer cb = command_buffers_.at(image_index);
  vk_check(vkResetCommandBuffer(cb, 0), "vkResetCommandBuffer");
  record_command_buffer(cb, sc, pl, framebuffers_.at(image_index));
  ...
  si.pWaitSemaphores = &image_available_[frame_index_];
  si.pWaitDstStageMask = &wait_stage;
  si.commandBufferCount = 1;
  si.pCommandBuffers = &cb;
  si.signalSemaphoreCount = 1;
  si.pSignalSemaphores = &render_finished_[frame_index_];
  ...
  pi.pWaitSemaphores = &render_finished_[frame_index_];
